...
import huxibianjie.com.gonggong.activity.WalkingActivity;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initTodayData() {
        CURRENT_STEP = SpUtil.getInstance().getInt("tempStep", 0);
        updateNotification();
    }
...
//        filter.addAction(Intent.ACTION_USER_PRESENT);
        ...
        mBatInfoReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(final Context context, final Intent intent) {
                ...
                if (SpUtil.getInstance().getBoolean("isEnd", false)) {
                    System.out.println("到达第八个");
                    isNewDay();
                } else if (Intent.ACTION_SCREEN_ON.equals(action)) {
                    System.out.println("到达第一个");
                } else if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                    System.out.println("到达第二个");
                    //改为60秒一存储
                    duration = 60000;
                } else if (Intent.ACTION_USER_PRESENT.equals(action)) {
                    System.out.println("到达第三个");
                    save();
                    //改为30秒一存储
                    duration = 30000;
                } else if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(intent.getAction())) {
                    System.out.println("到达第四个");
                    //保存一次
                    save();
                } else if (Intent.ACTION_SHUTDOWN.equals(intent.getAction())) {
                    System.out.println("到达第五个");
                    save();
                } else if (Intent.ACTION_DATE_CHANGED.equals(action)) {//日期变化步数重置为0
                    System.out.println("到达第六个");
                    save();
                    isNewDay();
                } else if (Intent.ACTION_TIME_CHANGED.equals(action)) {
                    //时间变化步数重置为0
                    System.out.println("到达第七个");
                    isCall();
                    save();
                    isNewDay();
    ...
    private void updateNotification() {
        ...
        Intent hangIntent = new Intent(this, WalkingActivity.class);
    ...
    public void onSensorChanged(SensorEvent event) {
        if (stepSensorType == Sensor.TYPE_STEP_COUNTER) {
            ...
            if (tempStep < 1) {
                ...
                if (SpUtil.getInstance().getLong(Constant.STEPNUMBER, 0) != 0) {
            ...
            if (!Constant.hasRecord) {
                Constant.hasRecord = true;
                SpUtil.getInstance().putInt(Constant.ONESTEPNUMBER, tempStep);
            }
            CURRENT_STEP = tempStep - SpUtil.getInstance().getInt(Constant.ONESTEPNUMBER, 0);
        } else if (stepSensorType == Sensor.TYPE_STEP_DETECTOR) {
            if (event.values[0] == 1.0) {
                CURRENT_STEP = SpUtil.getInstance().getInt(Constant.ONESTEPNUMBER, 0);
                CURRENT_STEP++;
                SpUtil.getInstance().putInt(Constant.ONESTEPNUMBER, CURRENT_STEP);
            }
        }
        updateNotification();
    }
    ...
    private void addBasePedometerListener() {
        ...
        mStepCount.initListener(new StepValuePassListener() {
            ...
            public void stepChanged(int steps) {
                SpUtil.getInstance().putInt(Constant.ONESTEPNUMBER, steps);
                CURRENT_STEP = steps;
                updateNotification();
            }
        });
    ...
    private void save() {
        ...
        SpUtil.getInstance().putInt("tempStep", tempStep);
    }
